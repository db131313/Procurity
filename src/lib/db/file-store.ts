...
/**
 * Upsert projects for the cities represented in `projects` (or explicit `cities`).
 * Other cities' rows are preserved — required for multi-metro sync.
 */
export async function replaceProjects(
  ...
  if (projects.length && cities.size) {
    const kept = db.projects.filter((p) => !cities.has(p.city));
    db.projects = [...kept, ...projects];
  }
  db.events = [...events, ...db.events].slice(0, 500);
